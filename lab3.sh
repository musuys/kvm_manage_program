...
deltemp=$(mktemp -t test.XXX)	# 삭제할 가상머신 정보를  담기 위한 변수
...
# 가상머신 삭제 함수
vmdelete(){		
	names=$(virsh list --all | grep -v Name | gawk '{print $2}' | sed '/^$/d')
	...
	dialog --title "가상머신 삭제 리스트" --radiolist "select one! " 15 50 5 $vm 2>$deltemp 
	delimage=$(cat $deltemp)	
	if [ $delimage ]
	then
	dialog --title "${delimage} 삭제창" --yesno "삭제하시겠습니까? " 10 20
		#if [ $? -eq 0 ]
		#then
		#virsh destoy $delimage
		#virsh undefine $delimage --remove-all-storage
		#fi
	else
		break
	fi
...
do
        ...
        dialog --menu "KVM 관리 시스템" 20 40 8 1 "가상머신 리스트" 2 "가상 네트워크 리스트" 3 "가상머신 생성" 4 "가상머신삭제" 0 "종료" 2> $ans
